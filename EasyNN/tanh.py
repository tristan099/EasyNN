...
class tanh:
    ...
    def backward(self, der, LR, mu, epoch, k, l1, l2):
        ...
        self.der = self.der*self.tander(self.z)@self.w.T
    ...
    def backwardada(self, der, LR, mu, ep, l1, l2):
        ...
        gradbw = LRw*(self.a.T@(self.der*self.tander(self.z)) +
                      l1*np.sign(self.w)+l2*self.w)
        gradbp = LRb*(np.sum(self.der*self.tander(self.z)) +
        ...
        self.momw = mu*self.momw-LRw * \
            (self.a.T@(self.der*self.tander(self.z))+l1*np.sign(self.w)+l2*self.w)
        self.momb = mu*self.momb-LRb * \
            (np.sum(self.der*self.tander(self.z))+l1*np.sign(self.b)+l2*self.b)

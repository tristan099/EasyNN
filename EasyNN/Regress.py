"""
Class for outputting a continuous variable in the case of regression.
"""
# lib
import numpy as np
...
class Regress:
    ...
    def __init__(self,indims,outdims):
        ...
        self.w=np.random.randn(self.indims,self.outdims)*np.sqrt(2/(self.indims+self.outdims))
        self.b=np.random.randn(1,self.outdims)*np.sqrt(2/(self.indims+self.outdims))
    ...
    def backward(self,LR,y,mu,epoch,k,l1,l2):
        LR=LR/(k*epoch+1)
        self.der=self.z-y
        self.momw=mu*self.momw-LR*(self.a.T@(self.der)+l1*np.sign(self.w)+l2*self.w)
        self.momb=mu*self.momb-LR*(np.sum(self.der)+l1*np.sign(self.b)+l2*self.b)
    ...
    def backwardada(self,LR,mu,ep,y,l1,l2):
        ...
        self.momw=mu*self.momw-LRw*(self.a.T@(self.der)+l1*np.sign(self.w)+l2*self.w)
        self.momb=mu*self.momb-LRb*(np.sum(self.der)+l1*np.sign(self.b)+l2*self.b)
    ...
    def backwardadam(self,LR,mu,ep,gam,y,epoch,l1,l2):
        ...
        self.mw=mu*self.mw+(1-mu)*(self.a.T@self.der+l1*np.sign(self.w)+l2*self.w)
        self.vw=gam*self.vw+(1-gam)*(self.a.T@self.der+l1*np.sign(self.w)+l2*self.w)**2
        self.mhat=self.mw/(1+mu**epoch)
        self.vhat=self.vw/(1+gam**epoch)

        self.mb=mu*self.mb+(1-mu)*LR*np.sum(self.der+l1*np.sign(self.b)+l2*self.b)
        self.vb=gam*self.vb+(1-gam)*LR*(np.sum(self.der)+l1*np.sign(self.b)+l2*self.b)**2
